...
# Define an AI-based aspect extraction model (NER for dynamic extraction)
aspect_extraction_model = pipeline("ner", model="dslim/bert-base-NER")
...
# Extract aspects from the question itself using AI
def extract_aspects_from_question(question):
    ner_results = aspect_extraction_model(question)
    aspects = set()

    for entity in ner_results:
        if entity['entity'].startswith('B-'):  # Identify entities as aspects
            aspects.add(entity['word'].strip())
    
    # If no specific aspects are found, return 'general'
...
def process_feedback(questions, feedback_text):
    ...
    for question in questions:
        ...
        # Step 1: Extract aspects from the question using AI
        aspects = extract_aspects_from_question(question)
        
        # Step 2: Analyze sentiment for the feedback text using AI
        sentiment_results = sentiment_classification_model(feedback)
        ...
        # Step 3: Calculate sentiment score using TextBlob (optional)
        textblob_sentiment_score = TextBlob(feedback).sentiment.polarity
